...
public:
    ...
    void Print(){
        ...
        for(int i = 0; i < PERIOD; i++) {
            ...
            std::cout << GetTextLine2(timeArray, i * 5) << std::endl;
...
private:
    ...
    std::string GetTextLine(std::vector<std::shared_ptr<Subject>> &timeArray, int start){
        ...
        for(int i = 0; i < COLUMN; i++){
            ...
            if(timeArray[start + i] != nullptr) {
                ...
                result +=  MidText(timeArray[start + i]->GetName()) ;
            }else{
                result += MidText("");
            }
        }
    ...
    std::string GetTextLine2(std::vector<std::shared_ptr<Subject>> &timeArray, int start){
        std::string result;

        for(int i = 0; i < COLUMN; i++){
            result += "+";
            if(timeArray[start + i] != nullptr) {
                result +=  MidText(timeArray[start + i]->GetClassroom()) ;
