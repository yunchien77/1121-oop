...
TEST(CheckPoint3, TestTrainConstructorShouldHaveCorrectTrain){
    ...
    std::shared_ptr<EnergyInfo> energyInfo = std::make_shared<DieselEnergy>(450);
...
TEST(CheckPoint3, TestTrainReserveSeatWithMultiplePollShouldHaveCorrectReserveInfo){
    std::shared_ptr<TrainInfo> trainInfo = std::make_shared<TrainInfo>(8, 45, 75, "ZhiChang", std::vector<std::string>{"Taipei", "Tainan"});
    std::shared_ptr<EnergyInfo> energyInfo = std::make_shared<DieselEnergy>(450);
    Train train(trainInfo, energyInfo);
...
TEST(CheckPoint3, TestTrainGetArrivalTimeShouldHaveCorrectArrivalTime){
    ...
    ASSERT_EQ(train.GetEnergyInfo()->GetTerminalStationArrivalTimeInSecond(), calculateHelper.CalculateDieselArrivalTime(450));
}
...
TEST(CheckPoint3, TestTrainGetDistanceShouldHaveCorrectDistance){
    ...
    CalculateHelper calculateHelper;
    ASSERT_EQ(train.GetEnergyInfo()->GetDistance(), calculateHelper.CalculateDieselDistance(450));
}