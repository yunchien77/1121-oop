...
TEST(CheckPoint4, TestLocalTrainConstructorShouldHaveCorrectTrain){
    ...
    std::shared_ptr<EnergyInfo> energyInfo = std::make_shared<ElectricEnergy>(450);
...
TEST(CheckPoint4, TestTrainGetArrivalTimeShouldHaveCorrectArrivalTime){
    ...
    ASSERT_EQ(train.GetEnergyInfo()->GetTerminalStationArrivalTimeInSecond(), calculateHelper.CalculateElectricArrivalTime(450));
}
...
TEST(CheckPoint4, TestLocalTrainGetDistanceShouldHaveCorrectDistance){
    ...
    CalculateHelper calculateHelper;
    ASSERT_EQ(train.GetEnergyInfo()->GetDistance(), calculateHelper.CalculateElectricDistance(450));
}
...
TEST(CheckPoint4, TestExpressTrainReserveSeatShouldHaveCorrectReserveInfo){
    std::shared_ptr<TrainInfo> trainInfo = std::make_shared<TrainInfo>(8, 45, 75, "ZhiChang", std::vector<std::string>{"Taipei", "HsinChu", "Tainan"});
    std::shared_ptr<EnergyInfo> energyInfo = std::make_shared<ElectricEnergy>(450);
    ExpressTrain train(trainInfo, energyInfo);
...
TEST(CheckPoint4, TestExpressTrainReserveSeatWithMultiplePollShouldHaveCorrectReserveInfo){
    std::shared_ptr<TrainInfo> trainInfo = std::make_shared<TrainInfo>(8, 45, 75, "ZhiChang", std::vector<std::string>{"Taipei", "HsinChu", "Tainan"});
    std::shared_ptr<EnergyInfo> energyInfo = std::make_shared<ElectricEnergy>(450);
    ExpressTrain train(trainInfo, energyInfo);
...
TEST(CheckPoint4, TestTouristTrainReserveSeatWithMultiplePollShouldHaveCorrectReserveInfo){
    std::shared_ptr<TrainInfo> trainInfo = std::make_shared<TrainInfo>(8, 45, 75, "ZhiChang", std::vector<std::string>{"Taipei", "Tainan"});
    std::shared_ptr<EnergyInfo> energyInfo = std::make_shared<ElectricEnergy>(450);
    TouristTrain train(trainInfo, energyInfo);
    for(int i = 0; i < 7*45; i++){
